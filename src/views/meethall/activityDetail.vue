  ...
  <el-main class="content">
    <el-card class="play-content"
      ...
      <el-col :span="16"
              class="play-right">
        <div>
          <div class="play-title">{{ item.newsTitle }}</div>
          <p class="play-status">
            <span class="p-ft_cw">发布时间: </span><span class="mr20">{{ item.newsReleaseDate }}</span>
          </p>
        </div>
...
export default {
  ...
  methods: {
    ...
    loadData (id) {
      ...
      let newsId = id ? id : this.$route.query.newsId
      this.loading=true
      getNews(newsId).then(res => {
        if(res.code=='0000') {
          ...
          this.item=res.obj
          if(!id){
            if(res.obj.otherNews.length>15){
              this.otherNews=res.obj.otherNews
