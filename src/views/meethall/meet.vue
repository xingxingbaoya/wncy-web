<template>
  <el-main class="main-layout">
    <el-row class="m-top">
      <div class="left-m">
        <img v-real-img="item.congressAvatar"
             :src="`${imgUrl}/ss2.jpg`"
             alt=""
             class="l-img">
      </div>
    ...
    <el-row class="m-meet">
      <el-card class="m-card"
               shadow="never"
               :body-style="{ padding: '40px 0 0' }">
        <div slot="header"
             class="clearfix">
          <span class="title">会议议程</span>
        </div>
        <div class="meet-tab"
             :class="{'line-after': isThirdHide }">
          <el-tabs v-model="activeSecondTab"
                   type="card">
            <el-tab-pane v-for="(item,index) in listData"
                         :label="item.meetTime"
                         :name="index">
              <el-row>
                <el-col v-for="it in item.forum"
                        class="s-layout"
                        :span="12"
                        @click.native="showDetail(it)">
                  <div class="s-meet">
                    <div class="s-color">
      ...
      <el-card class="m-card"
        ...
        <div slot="header"
          ...
          <el-button class="more"
                     type="text"
                     @click="goguest">查看更多<i class="el-icon-arrow-right" /></el-button>
        </div>
        <el-row>
          <el-col>
            <!-- <div class="persons">
            ...
            <el-col v-for="item in listGuest"
                    class="c-layout"
                    @click.native="godetail(item)"
                    :key="item">
              <div class="c-bg">
    ...
    <el-dialog :visible.sync="meetVisable"
      ...
      <el-row v-loading="loading"
        ...
        <el-divider><img class="m3"
               :src="`${imgUrl}/m3.png`"
               alt=""></el-divider>
        <el-row class="m-info">
          <el-col :span="12"
                  class="info-layout">
            <img :src="`${imgUrl}/md1.png`"
                 alt="md1"
                 class="image"><span class="m-t">会场：</span><span class="m-desc">{{ meetInfo.theVenue }}</span>
          </el-col>
          <el-col :span="12"
                  class="info-layout">
            <img :src="`${imgUrl}/md2.png`"
                 alt="md2"
                 class="image"><span class="m-t">主办单位：</span><span class="m-desc">{{ meetInfo.sponsor }}</span>
          </el-col>

          <el-col :span="12"
                  class="info-layout">
            <img :src="`${imgUrl}/md3.png`"
                 alt="md3"
                 class="image"><span class="m-t">承办单位：</span><span class="m-desc">{{ meetInfo.undertake | formatEmpty }}</span>
          </el-col>

          <el-col :span="12"
                  class="info-layout">
            <img :src="`${imgUrl}/md4.png`"
                 alt="md4"
                 class="image"><span class="m-t">协办单位：</span><span class="m-desc">{{ meetInfo.assistance | formatEmpty }}</span>
          </el-col>

          <el-col :span="12"
                  class="info-layout">
            <img :src="`${imgUrl}/md5.png`"
                 alt="md5"
                 class="image"><span class="m-t">支持单位：</span><span class="m-desc">{{ meetInfo.support | formatEmpty }}</span>
          </el-col>
          <el-col :span="12"
                  class="info-layout">
            <img :src="`${imgUrl}/md6.png`"
                 alt="md6"
                 class="image"><span class="m-t">时间：</span><span class="m-desc">{{ meetInfo.startTime + ' - ' + meetInfo.endTime }}</span>
          </el-col>
        ...
        <el-row class="c-bottom">
          <el-table :data="tableData"
                    ...
                    stripe
                    style="width: 100%">
            <el-table-column prop="agendaTime"
                             label="会议时间"
                             header-align="center"
                             :show-overflow-tooltip="true"
                             width="270" />
            <el-table-column prop="agendaSubject"
                             label="会议主题"
                             align="center" />
...
export default {
  ...
  methods: {
    loadData () {
      ...
      getTwoAll({ congressId: 1 }).then(res => {
        if(res.code=='0000') {
      ...
      findAllGuest({ ...formpage,congressId: 1 }).then(res => {
        if(res.code=='0000') {
          this.listGuest=res.rows
          this.listGuest.length=10
        } else {
          this.$message.warning(res.msg)
        }
      }).finally(() => {
