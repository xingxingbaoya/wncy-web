  ...
  <el-main>
    <el-card class="play-content" :body-style="{ padding:'30px' }">
      ...
      <el-col :span="18" class="play-right">
        <div class="play-desc">
          <div class="play-title pb30">{{ item.project && item.project.title }}</div>
          <p class="play-status mb24">
            <span class="label-t">项目进度：</span><span class="ps-color mr20">{{ formatStatus(item.tradeStatus) }}</span>
...
import { getTradeProcessDetail } from '@/api/tradinghall'
...
export default {
  ...
  methods: {
    loadData() {
      ...
      getTradeProcessDetail({ projectId }).then(res => {
        if (res.code == '0000') {
          this.item = res.obj
        } else {
