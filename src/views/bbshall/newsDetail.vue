...
import {  getZgcNewsDetail,getZgcNewsList } from '@/api/bbshall'
...
export default {
  ...
  methods: {
    ...
    loadData (id) {
      ...
      let newsId = id ? id : this.$route.query.id
      this.loading=true
      getZgcNewsDetail({id:newsId,pageSize:10,pageNum:1}).then(res => {
        if(res.code=='0000') {
          const regex=new RegExp('<img','gi')
          // res.obj.newsContent=res.obj.newsContent.replace(regex,`<img style="max-width: 100%; height: auto"`);
          this.item=res.obj
          if(!id){
            if(res.obj.newsList.length>15){
              this.otherNews=res.obj.newsList
              this.otherNews.length = 15
            }else{
              this.otherNews=res.obj.newsList.filter((item=>{
