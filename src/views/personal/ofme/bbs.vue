  ...
  <el-main class="m-layout">
    <el-row class="search-area">
      <div class="query-layout">
        <el-input v-model="params.keywords" placeholder="请输入关键字进行搜索" class="input-search">
          <el-button slot="append" @click="()=>{this.pager.pageNum = 1;this.loadData()}">搜索</el-button>
        </el-input>
        <el-select v-model="params.status" placeholder="审核状态" class="sel-layout">
          <el-option label="全部" value="" />
          <el-option
            v-for="item in statusTypeDict"
            :key="item.dictCode"
...
import { getMyManuscript } from '@/api/bbshall'
export default {
  name: 'Personalbbs',
  mixins: [common],
  data() {
  ...
  methods: {
    loadData() {
      ...
      getMyManuscript({ ...formpage, ...this.params }).then(res => {
        if (res.code == '0000') {
