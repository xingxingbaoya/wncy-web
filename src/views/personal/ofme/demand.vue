<template>
  <el-main class="m-layout">
    <el-row class="search-area">
      <el-input v-model="name" placeholder="请输入关键字进行搜索" class="input-search">
      ...
      <div class="c-bottom">
        ...
        >
          ...
          >
            <template slot-scope="scope">
              <span>{{ scope.row.province | MapName(scope.row.city,scope.row.county) }}</span>
            </template>
          </el-table-column>
...
export default {
  ...
  methods: {
    loadData() {
      ...
      getMyDemandList({ ...formpage, name: this.name }).then(res => {
        if (res.code == '0000') {
