        ...
        >
          ...
          >
            <template slot-scope="scope">
              <span>{{ scope.row.status | formatAsk }}</span>
            </template>
          </el-table-column>
...
import { getMyAnswers, getMyIndustry, getMyManuscript, getMyQuestion } from '@/api/bbshall'
export default {
  name: 'Personalask',
  mixins: [common],
  data() {
  ...
  methods: {
    loadData() {
      ...
      getMyAnswers({ ...formpage, title: this.title }).then(res => {
        if (res.code == '0000') {
    ...
    godetail(row) {
      ...
      this.$router.push({
        path: '/personal/center/detail-ask',
        query: {
          questionId,
