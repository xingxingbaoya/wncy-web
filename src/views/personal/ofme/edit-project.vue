      ...
      getProjectDetail({ id })
        .then((res) => {
          if (res.code == "0000") {
            ...
            if (res.obj.logo) {
              // this.formData.pic_file = res.obj.logo;
              this.pic_file_list = [
                {
                  name: this.getFileNameFromUrl(res.obj.logo),
                  url: res.obj.logo,
                  isUrl: true,
                },
              ];
            ...
            if (res.obj.attachment) {
              // this.formData.attachment_file = res.obj.attachment;
              this.file_list = [
                {
                  name: this.getFileNameFromUrl(res.obj.attachment),
                  url: res.obj.attachment,
                  isUrl: true,
                },
              ];
            }
        ...
        try {
          if (vali) {
            ...
            const res = await addProjectDetail(
              param2Form({
                ...this.formData,
                attachment_file: this.file_list[0].isUrl
                  ? this.file_list[0].url
                  : this.file_list[0],
                pic_file: this.pic_file_list[0].isUrl
                  ? this.pic_file_list[0]?.url
                  : this.pic_file_list[0]?.raw || "",
              })
            );
            if (res.code == "0000") {
