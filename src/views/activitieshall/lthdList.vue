<template>
  <el-main class="content">
    <el-row class="search-area">
      <el-input v-model="searchData.title" placeholder="请输入关键字进行搜索" class="input-search">
    ...
    <el-row v-loading="loading" class="search-content">
      <!-- <el-tabs v-model="searchData.actClassification" @tab-click="()=>{this.loadData();this.rangeTime = [];this.searchData.actStatus = ''}"> -->
        ...
        <el-row class="search-query pb26">
          ...
          />
          <el-button type="primary" class="clear-query" @click="clearData">清空</el-button>
        </el-row>
            ...
            <el-row>
              <el-col v-for="(o,index) in listData" :span="6">
                <el-card :body-style="{ padding: '0px' }" class="inner_card" :class="clearMargin(index)" shadow="hover" @click.native="goaction(o)">
                  <img v-real-img="o.actCover" :src="`${imgUrl}/activity${kidx+1}.jpg`" class="image">
                  <el-tag v-show="o.upType == 'video'" effect="plain" type="info" size="small" :class="{'status': o.actStatus == 'started'}">
                    {{ actionStat(o) }}
                  </el-tag>
                  <img v-show="o.upType == 'video'" :src="`${imgUrl}/play.png`" class="play">
...
export default {
  ...
  data() {
    return {
      searchData: {
        title: this.$route.query.searchTitle,
